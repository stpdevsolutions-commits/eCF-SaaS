...
function EcfDetailContent() {
  ...
  const [actionLoading, setActionLoading] = useState<
    'validate' | 'sign' | 'transmit' | 'status' | 'cancel' | null
  >(null);
  ...
  async function handleTransmit() {
    setActionLoading('transmit');
    setActionMsg(null);
    try {
      const result = await transmitEcf(id);
      setActionMsg({
        type: 'success',
        text: `Transmitido a la DGII. UUID: ${result.uuid} — ${(result.mensajes ?? []).join(', ')}`,
      });
      await loadEcf();
    } catch (err: unknown) {
      setActionMsg({
        type: 'error',
        text: err instanceof Error ? err.message : 'Error al transmitir',
      });
    } finally {
      setActionLoading(null);
    }
  ...
  async function handleCheckStatus() {
    ...
    setActionMsg(null);
    try {
      const result = await checkEcfStatus(id);
      setActionMsg({
        type: 'success',
        text: `Estado en la DGII: ${result.estadoDgii} — ${result.mensaje}`,
      });
      await loadEcf();
    } catch (err: unknown) {
      setActionMsg({
        type: 'error',
        text: err instanceof Error ? err.message : 'Error al consultar estado',
      });
    } finally {
      setActionLoading(null);
    }
  ...
  async function handleCancel() {
    if (!motivoCancelacion.trim()) {
      setActionMsg({ type: 'error', text: 'Debes indicar un motivo de cancelación.' });
      return;
    }
    setActionLoading('cancel');
    setActionMsg(null);
    try {
      const result = await cancelEcf(id, motivoCancelacion.trim());
      setActionMsg({ type: 'success', text: result.mensaje });
  ...
  const canTransmit = ecf && ecf.estado === 'signed';
              ...
              {showCancelForm && (
                <div className="mt-4 p-4 bg-red-50 border border-red-200 rounded-lg space-y-3">
                  ...
                  <div className="flex gap-2">
                    ...
                    >
                      {actionLoading === 'cancel' ? 'Cancelando…' : 'Confirmar cancelación'}
                    </button>
                  </div>
                </div>
              )}
